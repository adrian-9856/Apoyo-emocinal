/**
 * =========================================================================
 * SISTEMA DE APOYO EMOCIONAL - VERSIÓN FINAL
 * Versión: 4.0 - Código 100% funcional y limpio
 * =========================================================================
 *
 * INSTALACIÓN RÁPIDA:
 * 1. Copiar TODO este código
 * 2. Apps Script → Pegar
 * 3. Cambiar email del director (línea 698)
 * 4. Guardar
 * 5. Ejecutar: instalarSistemaCompleto
 * 6. Crear trigger: onEditSistema (Al editar)
 *
 * HOJAS: 7 (sin Asistencia Grupal)
 * COLUMNAS EN TERAPIAS: 8
 *
 * =========================================================================
 */
...
function crearHojaListaEspera() {
  ...
  const widths = [110, 60, 200, 120, 100, 100, 250, 150, 180, 120, 200, 100];
...
function configurarValidaciones() {
  ...
  const accionRule = SpreadsheetApp.newDataValidation()
    ...
    .setAllowInvalid(false)
    .build();
  listaEspera.getRange("L2:L200").setDataValidation(accionRule);
}
...
// =========================================================================
// TRIGGER PRINCIPAL - 3 AUTOMATIZACIONES
// =========================================================================
...
function onEditSistema(e) {
  try {
    ...
    // AUTOMATIZACIÓN 1: Enviar de Lista de Espera (columna 12)
...
function procesarEnvioListaEspera(sheetOrigen, fila) {
  try {
    ...
    const rangoEdad = datos[5];
    const malestar = datos[6];
    const derivadoPor = datos[7];
    const contacto = datos[8];
...
function procesarAsignacion(sheetOrigen, fila, terapeuta) {
  try {
    ...
    terapias.getRange(nuevaFila, 1, 1, 8).setValues([nuevaAsignacion]);
